...
export default function Index() {
  ...
  return <ClientOnly>{() => 
    <div className="flex flex-col h-screen items-center justify-center gap-4 bg-slate-800 text-slate-100">
      <h1 className="text-3xl font-semibold tracking-wide">Day #{state.context.day}</h1>
      ...
      <table className="table-fixed w-9/12 mx-auto">
        <thead>
          <tr className="border-b border-gray-600">
            ...
            <th className="text-center pb-2">Copper</th>
            <th className="text-center pb-2">Wheat</th>
            <th className="text-center pb-2">Olive</th>
          </tr>
        </thead>
        <tbody>
          {ports.map(port => (
            <tr key={port} className="hover:bg-gray-900 cursor-crosshair">
              ...
              <td className="py-2.5 hover:text-yellow-400 text-center tabular-nums">${state.context.prices[port].copper}</td>
              <td className="py-2.5 hover:text-yellow-400 text-center tabular-nums">${state.context.prices[port].wheat}</td>
              <td className="py-2.5 hover:text-yellow-400 text-center tabular-nums">${state.context.prices[port].olive}</td>
            </tr>
          ))}
          <tr className="border-t border-gray-400 text-lg">
            <td className="pt-4 text-left">Time to next update</td>
            <td colSpan={2}></td>
            <td className="pt-4 text-center text-lg">{state.context.nextPriceChange} days</td>
          </tr>
        </tbody>
      </table>
      <div className="mt-9 w-9/12 flex items-center justify-between">
        <div>
          <h2 className="text-xl">{state.context.destination ? `Traveling to ${state.context.destination}..` : 'Travel'}</h2>
          ...
          <div className={["mt-7 flex gap-2 items-center border-t py-3 px-1", state.context.event ? "" : "invisible"].join(' ')}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
              <path strokeLinecap="round" strokeLinejoin="round" d="m11.25 11.25.041-.02a.75.75 0 0 1 1.063.852l-.708 2.836a.75.75 0 0 0 1.063.853l.041-.021M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9-3.75h.008v.008H12V8.25Z" />
            </svg>
            <p className="">{state.context.event}</p>
